...
function hs_initalize_atwho()
{
	?>
	<script>
	atwho.setMembersSource([
		{ name: 'users', trigger: '@' },
		{ name: 'tags', trigger: '#' source: '/wp-admin/admin-ajax.php?action=search_book_tags' }
	]);
	</script>
	<?php
}
add_action('wp_footer', 'hs_initalize_atwho');
